...
SPLIT_VERSE_MIN_LINES = 2
KINDLE_VIEWPORT_WIDTH = 1080
KINDLE_VIEWPORT_HEIGHT = 1230
KINDLE_BOTTOM_PADDING = 20
PIXEL_PAGE_SAFETY = 10
...
def split_children_on_br(node: Tag) -> list[str]:
    lines: list[str] = []
    current: list[str] = []
...
def paragraph_verse_lines(node: Tag) -> tuple[str, str | None, list[str]]:
    verse = node.select_one(".verse-line, .verse-continuation")
    if not isinstance(verse, Tag):
        return "", None, []

    sup = verse.find("sup", recursive=False)
    body = None
    for child in verse.find_all("span", recursive=False):
        if "firstIndex" not in child.get("class", []):
            body = child
            break
    if not isinstance(body, Tag):
        body = verse

    lines = split_children_on_br(body)
    if len(lines) < SPLIT_VERSE_MIN_LINES:
        return "", None, []
    verse_number = str(sup) if isinstance(sup, Tag) else None
    return " ".join(node.get("class", [])), verse_number, lines


def render_split_verse_paragraph(original_classes: str, verse_number: str | None, lines: list[str]) -> str:
    classes = [class_name for class_name in original_classes.split() if class_name not in {"split-block", "split-verse-block"}]
    classes.extend(["split-block", "split-verse-block"])
    class_attr = html.escape(" ".join(classes), quote=True)
    if verse_number:
        body = f'<span class="verse-line">{verse_number} <span>{"<br/>".join(lines)}</span></span>'
    else:
        body = f'<span class="verse-continuation">{"<br/>".join(lines)}</span>'
    return f'<p class="{class_attr}">{body}</p>'


def paragraph_nested_lines(node: Tag) -> list[str]:
    if not node.find("br"):
        return []
    if node.select_one(".verse-line, .verse-continuation"):
        return []
    ...
    if len(meaningful_children) == 1 and isinstance(meaningful_children[0], Tag):
        lines = split_children_on_br(meaningful_children[0])
        return lines if len(lines) >= SPLIT_VERSE_MIN_LINES else []

    lines = split_children_on_br(node)
    return lines if len(lines) >= SPLIT_VERSE_MIN_LINES else []


def antiphon_parts(node: Tag) -> tuple[str, str | None, list[str]]:
    if node.name != "div" or "antiphon" not in node.get("class", []):
        return "", None, []

    body = node.find(class_="body")
    if not isinstance(body, Tag):
        return "", None, []
    ...
    pre = node.find(class_="pre")
    pre_html = str(pre) if isinstance(pre, Tag) else None
    return " ".join(node.get("class", [])), pre_html, lines


def render_split_antiphon(original_classes: str, pre_html: str | None, lines: list[str]) -> str:
    classes = [class_name for class_name in original_classes.split() if class_name != "split-block"]
    classes.append("split-block")
    class_attr = html.escape(" ".join(classes), quote=True)
    pre = f"\n{pre_html}" if pre_html else ""
    return f'<div class="{class_attr}">{pre}\n<span class="body">{"<br/>".join(lines)}</span>\n</div>'
...
def paragraph_wrapped_text_tokens(node: Tag) -> tuple[str, list[str]]:
    if node.find("br"):
        return "", []
    ...
    wrapper = meaningful_children[0]
    if not wrapper.get("class"):
        return "", []
    ...
    return " ".join(wrapper.get("class", [])), tokens
...
def render_split_wrapped_tokens_paragraph(node: Tag, wrapper_classes: str, tokens: list[str]) -> str:
    classes = [class_name for class_name in node.get("class", []) if class_name != "split-block"]
    classes.append("split-block")
    class_attr = html.escape(" ".join(classes), quote=True)
    wrapper_attr = html.escape(wrapper_classes, quote=True)
    return f'<p class="{class_attr}"><span class="{wrapper_attr}">{"".join(tokens).strip()}</span></p>'
...
def split_wrapped_text_paragraph_to_fit(paragraph: Tag, remaining_units: int) -> tuple[str, str] | None:
    if remaining_units < 4:
        return None

    allowed_units = remaining_units + 2
    wrapper_classes, tokens = paragraph_wrapped_text_tokens(paragraph)
    if len(tokens) < 8:
        return None

    best_cut = 0
    for cut in range(1, len(tokens)):
        prefix = render_split_wrapped_tokens_paragraph(paragraph, wrapper_classes, tokens[:cut])
        if block_units(prefix) <= allowed_units:
            best_cut = cut
        else:
            break
    ...
    suffix = render_split_wrapped_tokens_paragraph(paragraph, wrapper_classes, tokens[best_cut:])
    if not BeautifulSoup(suffix, "lxml").get_text(" ", strip=True):
        return None

    prefix = render_split_wrapped_tokens_paragraph(paragraph, wrapper_classes, tokens[:best_cut])
    return prefix, suffix
...
def split_block_to_fit(block_html: str, remaining_units: int) -> tuple[str, str] | None:
    ...
    soup = BeautifulSoup(block_html, "lxml")
    antiphon = soup.find("div", class_="antiphon")
    if isinstance(antiphon, Tag):
        original_classes, pre_html, antiphon_lines = antiphon_parts(antiphon)
        if antiphon_lines:
            best_cut = 0
            for cut in range(1, len(antiphon_lines)):
                prefix = render_split_antiphon(original_classes, pre_html, antiphon_lines[:cut])
            ...
            if best_cut > 0:
                return (
                    render_split_antiphon(original_classes, pre_html, antiphon_lines[:best_cut]),
                    render_split_antiphon(original_classes, None, antiphon_lines[best_cut:]),
                )
    ...
    if not lines:
        original_classes, verse_number, verse_lines = paragraph_verse_lines(paragraph)
        if verse_lines:
            best_cut = 0
            for cut in range(1, len(verse_lines)):
                prefix = render_split_verse_paragraph(original_classes, verse_number, verse_lines[:cut])
                if block_units(prefix) <= remaining_units:
                    best_cut = cut
                else:
                    break
            if best_cut > 0:
                prefix = render_split_verse_paragraph(original_classes, verse_number, verse_lines[:best_cut])
                suffix = render_split_verse_paragraph(original_classes, None, verse_lines[best_cut:])
                return prefix, suffix
        nested_lines = paragraph_nested_lines(paragraph)
        if nested_lines:
            best_cut = 0
            for cut in range(1, len(nested_lines)):
                prefix = render_split_paragraph(paragraph, nested_lines[:cut])
                if block_units(prefix) <= remaining_units:
                    best_cut = cut
                else:
                    break
            if best_cut > 0:
                return render_split_paragraph(paragraph, nested_lines[:best_cut]), render_split_paragraph(paragraph, nested_lines[best_cut:])
        wrapped_split = split_wrapped_text_paragraph_to_fit(paragraph, remaining_units)
        if wrapped_split:
            return wrapped_split
...
def annotate_page_block(block_html: str, index: int) -> str:
    soup = BeautifulSoup(block_html, "lxml")
    element = soup.find(["h1", "h2", "h3", "p", "div", "section"])
    if not isinstance(element, Tag):
        return f'<p data-page-block="{index}">{html.escape(BeautifulSoup(block_html, "lxml").get_text(" ", strip=True))}</p>'
    element["data-page-block"] = str(index)
    return str(element)


def inline_measure_shell(
    title: str,
    body: str,
    updated: str,
    liturgical_day: LiturgicalDay | None,
...
def measure_pixel_capacity(
    ...
    page.set_content(
        inline_measure_shell(title, "", updated, liturgical_day, nav, css, show_metadata, show_title, page_note),
        wait_until="load",
    )
    return float(
        page.evaluate(
            """() => {
                ...
                const outerHeight = (element) => {
                    ...
                    const marginTop = element.matches('.page-nav:last-child') ? 0 : parseFloat(style.marginTop) || 0;
                    const marginBottom = parseFloat(style.marginBottom) || 0;
                    return rect.height + marginTop + marginBottom;
                };
                const occupied = children.reduce((total, child) => total + outerHeight(child), 0);
                return Math.max(0, main.getBoundingClientRect().height - occupied);
            }"""
        )
    )


def measure_pixel_block_heights(
    page,
    blocks: list[str],
    title: str,
    updated: str,
    ...
    marked_blocks = "\n".join(annotate_page_block(block, index) for index, block in enumerate(blocks))
    page.set_content(
        inline_measure_shell(title, marked_blocks, updated, None, nav, css, show_metadata=False, show_title=False),
        wait_until="load",
    )
    measurements = page.evaluate(
        """() => Array.from(document.querySelectorAll('[data-page-block]')).map((element) => {
            const style = getComputedStyle(element);
            const rect = element.getBoundingClientRect();
            return {
                index: Number(element.dataset.pageBlock),
                height: rect.height + (parseFloat(style.marginTop) || 0) + (parseFloat(style.marginBottom) || 0)
            };
        })"""
    )
    heights = [0.0 for _ in blocks]
    for item in measurements:
        index = int(item["index"])
        if 0 <= index < len(heights):
            heights[index] = float(item["height"])
    return heights


def measure_pixel_block_height(
    page,
    block: str,
    title: str,
    updated: str,
    ...
    heights = measure_pixel_block_heights(page, [block], title, updated, nav, css)
    return heights[0] if heights else 0.0


def split_block_to_fit_by_pixels(
    block_html: str,
    max_height: float,
    page,
    title: str,
    ...
    if isinstance(antiphon, Tag):
        ...
        if antiphon_lines:
            best_cut = 0
            for cut in range(1, len(antiphon_lines)):
                candidate = render_split_antiphon(original_classes, pre_html, antiphon_lines[:cut])
                if measure_pixel_block_height(page, candidate, title, updated, nav, css) <= max_height:
                    best_cut = cut
                else:
                    break
            if best_cut <= 0 or best_cut >= len(antiphon_lines):
                return None
            return (
                render_split_antiphon(original_classes, pre_html, antiphon_lines[:best_cut]),
                render_split_antiphon(original_classes, None, antiphon_lines[best_cut:]),
    ...
    if lines:
        ...
        for cut in range(1, len(lines)):
            candidate = render_split_paragraph(paragraph, lines[:cut])
            if measure_pixel_block_height(page, candidate, title, updated, nav, css) <= max_height:
                best_cut = cut
            else:
                break
        if best_cut <= 0 or best_cut >= len(lines):
            return None
        return render_split_paragraph(paragraph, lines[:best_cut]), render_split_paragraph(paragraph, lines[best_cut:])

    original_classes, verse_number, verse_lines = paragraph_verse_lines(paragraph)
    if verse_lines:
        best_cut = 0
        for cut in range(1, len(verse_lines)):
            candidate = render_split_verse_paragraph(original_classes, verse_number, verse_lines[:cut])
            if measure_pixel_block_height(page, candidate, title, updated, nav, css) <= max_height:
                best_cut = cut
            else:
                break
        if best_cut <= 0 or best_cut >= len(verse_lines):
            return None
        return (
            render_split_verse_paragraph(original_classes, verse_number, verse_lines[:best_cut]),
            render_split_verse_paragraph(original_classes, None, verse_lines[best_cut:]),
        )
    ...
    if nested_lines:
        best_cut = 0
        for cut in range(1, len(nested_lines)):
            candidate = render_split_paragraph(paragraph, nested_lines[:cut])
            if measure_pixel_block_height(page, candidate, title, updated, nav, css) <= max_height:
                best_cut = cut
            else:
                break
        if best_cut <= 0 or best_cut >= len(nested_lines):
            return None
        return (
            render_split_paragraph(paragraph, nested_lines[:best_cut]),
            render_split_paragraph(paragraph, nested_lines[best_cut:]),
        )

    wrapper_classes, wrapped_tokens = paragraph_wrapped_text_tokens(paragraph)
    if len(wrapped_tokens) >= 8:
        low = 1
        high = len(wrapped_tokens) - 1
        best_cut = 0
        while low <= high:
            cut = (low + high) // 2
            candidate = render_split_wrapped_tokens_paragraph(paragraph, wrapper_classes, wrapped_tokens[:cut])
            if measure_pixel_block_height(page, candidate, title, updated, nav, css) <= max_height:
                best_cut = cut
                low = cut + 1
            else:
        ...
        if best_cut > 0 and best_cut < len(wrapped_tokens):
            return (
                render_split_wrapped_tokens_paragraph(paragraph, wrapper_classes, wrapped_tokens[:best_cut]),
                render_split_wrapped_tokens_paragraph(paragraph, wrapper_classes, wrapped_tokens[best_cut:]),
            )
    ...
    preferred_cut = best_cut
    for punctuation in (r"[.!?][”\"]?$", r"[;:,”“]$"):
        for cut in range(best_cut, max(0, best_cut - 40), -1):
            candidate = render_split_tokens_paragraph(paragraph, tokens[:cut])
            text = BeautifulSoup(candidate, "lxml").get_text(" ", strip=True)
            if re.search(punctuation, text) and measure_pixel_block_height(page, candidate, title, updated, nav, css) <= max_height:
                preferred_cut = cut
                break
        if preferred_cut != best_cut:
            break
    ...
    return (
        render_split_tokens_paragraph(paragraph, tokens[:preferred_cut]),
        render_split_tokens_paragraph(paragraph, tokens[preferred_cut:]),
    )


def split_block_candidates(block_html: str) -> list[tuple[str, str]]:
    soup = BeautifulSoup(block_html, "lxml")
    antiphon = soup.find("div", class_="antiphon")
    if isinstance(antiphon, Tag):
        original_classes, pre_html, antiphon_lines = antiphon_parts(antiphon)
        return [
            (
                render_split_antiphon(original_classes, pre_html, antiphon_lines[:cut]),
                render_split_antiphon(original_classes, None, antiphon_lines[cut:]),
            )
            for cut in range(1, len(antiphon_lines))
        ]
    ...
    if lines:
        for cut in range(1, len(lines)):
            candidates.append((render_split_paragraph(paragraph, lines[:cut]), render_split_paragraph(paragraph, lines[cut:])))
        return candidates
    ...
    if verse_lines:
        for cut in range(1, len(verse_lines)):
            candidates.append(
                (
                    render_split_verse_paragraph(original_classes, verse_number, verse_lines[:cut]),
                    render_split_verse_paragraph(original_classes, None, verse_lines[cut:]),
                )
            )
        return candidates
    ...
    if nested_lines:
        for cut in range(1, len(nested_lines)):
            candidates.append((render_split_paragraph(paragraph, nested_lines[:cut]), render_split_paragraph(paragraph, nested_lines[cut:])))
        return candidates

    wrapper_classes, wrapped_tokens = paragraph_wrapped_text_tokens(paragraph)
    if len(wrapped_tokens) >= 8:
        for cut in range(1, len(wrapped_tokens)):
            candidates.append(
                (
                    render_split_wrapped_tokens_paragraph(paragraph, wrapper_classes, wrapped_tokens[:cut]),
                    render_split_wrapped_tokens_paragraph(paragraph, wrapper_classes, wrapped_tokens[cut:]),
                )
            )
        return candidates
    ...
    if len(tokens) >= 8:
        for cut in range(1, len(tokens)):
            candidates.append((render_split_tokens_paragraph(paragraph, tokens[:cut]), render_split_tokens_paragraph(paragraph, tokens[cut:])))
    return candidates
...
def page_blocks_fit(
    ...
    page.set_content(
        inline_measure_shell(
            ...
            nav,
            css,
            show_metadata=page_index == 0,
            show_title=page_index == 0,
            page_note="" if page_index == 0 else "Trang 99/99",
        ),
        wait_until="load",
    )
    ...
    return metrics["scrollHeight"] <= KINDLE_VIEWPORT_HEIGHT and metrics["navBottom"] <= KINDLE_VIEWPORT_HEIGHT - KINDLE_BOTTOM_PADDING + 1


def rebalance_pixel_pages(
    pages: list[list[str]],
    page,
    title: str,
    updated: str,
    ...
    while index < len(pages) - 1:
        ...
        next_block = pages[index + 1][0]
        if page_blocks_fit(page, pages[index] + [next_block], index, title, updated, liturgical_day, nav, css):
            pages[index].append(pages[index + 1].pop(0))
            changed = True
        else:
            best: tuple[str, str] | None = None
            for prefix, suffix in split_block_candidates(next_block):
                if page_blocks_fit(page, pages[index] + [prefix], index, title, updated, liturgical_day, nav, css):
                    best = (prefix, suffix)
                else:
                    break
            if best:
    ...
    return [page_blocks for page_blocks in pages if page_blocks]


def paginate_html_by_pixels(
    fragment: str,
    title: str,
    updated: str,
    liturgical_day: LiturgicalDay | None,
    ...
    first_capacity = measure_pixel_capacity(
        page,
        title,
        updated,
        liturgical_day,
    ...
    next_capacity = measure_pixel_capacity(
        page,
        title,
        updated,
        liturgical_day,
        ...
        show_metadata=False,
        show_title=False,
        page_note="Trang 99/99",
    )
    if first_capacity <= 0 or next_capacity <= 0:
        return None

    block_heights = measure_pixel_block_heights(page, blocks, title, updated, nav, css)
    pages: list[list[str]] = []
    current: list[str] = []
    current_height = 0.0
    pending = list(zip(blocks, block_heights))

    while pending:
        block, height = pending.pop(0)
        height = height or 1.0
        capacity = (first_capacity if not pages else next_capacity) - PIXEL_PAGE_SAFETY
        if current and is_heading_block(block) and current_height >= capacity * 0.92:
            pages.append(current)
            current = []
            current_height = 0.0
            capacity = next_capacity - PIXEL_PAGE_SAFETY
        if current and current_height + height > capacity:
            split = split_block_to_fit_by_pixels(block, capacity - current_height, page, title, updated, nav, css)
            if split:
                prefix, suffix = split
                current.append(prefix)
                pages.append(current)
                suffix_height = measure_pixel_block_height(page, suffix, title, updated, nav, css)
                pending.insert(0, (suffix, suffix_height))
                current = []
                current_height = 0.0
                continue
            pages.append(current)
            current = []
            current_height = 0.0
            pending.insert(0, (block, height))
            continue
        if not current and height > capacity:
            split = split_block_to_fit_by_pixels(block, capacity, page, title, updated, nav, css)
            if split:
                prefix, suffix = split
                pages.append([prefix])
                suffix_height = measure_pixel_block_height(page, suffix, title, updated, nav, css)
    ...
    pages = rebalance_pixel_pages(pages, page, title, updated, liturgical_day, nav, css)
    return ["\n".join(page_blocks) for page_blocks in pages if page_blocks]


def paginate_prayers(
    prayers: list[Prayer],
    updated: str,
    liturgical_day: LiturgicalDay | None,
) -> dict[str, list[str]]:
    fallback = {prayer.slug: paginate_html(prayer.body_html) for prayer in prayers}
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        logging.warning("Playwright is not installed; using unit-based pagination")
        return fallback
    ...
    try:
        with sync_playwright() as playwright:
            ...
            context = browser.new_context(
                viewport={"width": KINDLE_VIEWPORT_WIDTH, "height": KINDLE_VIEWPORT_HEIGHT},
                device_scale_factor=1,
            )
            page = context.new_page()
            measured: dict[str, list[str]] = {}
            placeholder_nav = page_nav_html("previous.html", "next.html", 1, 99, "index.html")
            for prayer in prayers:
                pages = paginate_html_by_pixels(
                    prayer.body_html,
                    prayer.title,
                    updated,
                    liturgical_day,
                ...
                measured[prayer.slug] = pages if pages else fallback[prayer.slug]
            browser.close()
            logging.info("Using Playwright pixel pagination for Kindle pages")
            return measured
    except Exception as exc:
        logging.warning("Playwright pagination failed; using unit-based pagination: %s", exc)
        return fallback
...
"""
    ...
    paginated = paginate_prayers(ordered, updated, liturgical_day)
    for index, prayer in enumerate(ordered):
